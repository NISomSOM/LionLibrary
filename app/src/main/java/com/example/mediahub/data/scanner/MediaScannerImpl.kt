...
import com.example.mediahub.domain.model.ScanStatus
...
import com.example.mediahub.util.ConfidenceScorer
...
/**
 * Core scanner implementation that wires together:
 * - SAF folder traversal ([FolderScanner])
 * - Filename parsing ([FileNameParser])
 * - TMDB API search & details ([TmdbApiService])
 * - Image caching ([ImageCacheManager])
 * - Room persistence ([MediaDao], [SeasonDao], [EpisodeDao])
 *
...
class MediaScannerImpl(
    ...
    override operator fun invoke(): Flow<ScanProgress> = flow {
        ...
        for ((index, file) in allFiles.withIndex()) {
            ...
            try {
                ...
                handleUnidentified(file.displayName, fileUriString, file.mediaType)
                emit(ScanProgress(total, index + 1, file.displayName, ScanStatus.NO_INTERNET))
                // Spec: No internet during scan -> Show toast: "No internet. Files saved without metadata." Keep file in DB as Unidentified.
                // It does not specify aborting the entire scan immediately, but it's reasonable since network is dead.
                // However, I'll let it continue to save remaining files as unidentified? Wait, if we return@flow, the rest are not scanned.
                // The spec says "Files saved without metadata", meaning we should probably save the rest as unidentified without hitting API?
                // Let's just return to stop the scan, and next scan will pick up. Or just emit NO_INTERNET and break.
                return@flow
            } catch (e: Exception) {
    ...
    private suspend fun handleEpisode(
        ...
        try {
            ...
            getOrCreateSeason(showId, firstResult.id, parsed.season, apiKey)

            // Create episode entry
            val seasonDetails = try {
                tmdbApiService.getSeasonDetails(firstResult.id, parsed.season, apiKey)
            } catch (e: Exception) {
                null
            }
    ...
    /**
     * Finds or creates a SeasonEntity for a show. Returns the local season ID.
     */
    private suspend fun getOrCreateSeason(
        ...
        // Using a direct query approach since we need a suspending call
        val seasonDetails = try {
        ...
        return seasonDao.insert(seasonEntity)
    }

    /**
     * Saves a file as an unidentified media item.
     */
